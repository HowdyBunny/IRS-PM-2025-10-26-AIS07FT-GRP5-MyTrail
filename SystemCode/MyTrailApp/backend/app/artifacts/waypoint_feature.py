...
class RouteFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def _one_route_features(self, r: Dict[str, Any]) -> Dict[str, float]:
        ...
        # ----------------------------
        # ✅ 1. Basic attributes (supports nested route_info structure)
        # ----------------------------
        route_info = r.get("route_info", {})
        distance = r.get("distance", route_info.get("distance"))
        duration = r.get("duration", route_info.get("duration"))
        viewport = (r.get("geometry") or {}).get("viewport") or route_info.get("viewport")

        f["distance_m"] = _safe_float(distance)
        f["duration_s"] = _to_seconds(duration)
        ...
        # ----------------------------
        # ✅ 3. Waypoint aggregation (compatible with newer structure)
        # ----------------------------
        wps = r.get("waypoints")
        # If waypoints is a dict (newer structure)
        ...
        ratings = [
            ...
            for w in wps
            if isinstance(w, dict) and w.get("rating") is not None
        ]
        ...
        # ----------------------------
        # ✅ 4. Waypoint coordinate distance features
        # ----------------------------
        coords: List[Tuple[float, float]] = []
        for w in wps:
            loc = (w.get("location") if isinstance(w, dict) else {}) or {}
        ...
        # ----------------------------
        # ✅ 5. Waypoint declared distance aggregates
        # ----------------------------
        declared_dists = [
            _safe_float(w.get("distance_km"))
            for w in wps
            if isinstance(w, dict) and w.get("distance_km") is not None
        ]
        ...
        # ----------------------------
        # ✅ 6. Category / search category distribution and entropy
        # ----------------------------
